    ...
    return (
      ...
      >
            ...
            {selected.map((option) => {
              return (
                <Badge
                  key={option.value}
                  className={cn(
                    "w-full rounded-md bg-slate-400 py-2 text-center data-[disabled]:bg-muted-foreground data-[disabled]:text-muted data-[disabled]:hover:bg-muted-foreground",
        ...
        <div className="relative">
          {open && (
            <CommandList className="absolute top-1 z-10 w-2/3 rounded-md border bg-popover text-popover-foreground shadow-md outline-none animate-in">
              {isLoading ? (
                <>{loadingIndicator}</>
              ) : (
                <>
